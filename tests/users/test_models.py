...
def test_base_user_email(base_user):
...
def test_super_user_email_is_normalized(super_user):
    """Test that an admin email is normalized"""
    email = "alpha@REALESTATE.COM"
    assert super_user.email() == email.lower()


def test_super_user_is_not_staff(user_factory):
    """Test that an error is rasied when an admin user has is_staff set to False"""
...
def test_create_user_with_no_first_name(user_factory):
    """Test that creating a new user with no first name raises an error"""
    with pytest.raises(ValueError) as err:
        user_factory.create(first_name=None)
    assert str(err.value) == "You must submit a first name"


def test_create_user_with_no_last_name(user_factory):
    """Test that creating a new user with no last name raises an error"""
    with pytest.raises(ValueError) as err:
        user_factory.create(last_name=None)
    assert str(err.value) == "You must submit a last name"


def test_create_superuser_with_no_email(user_factory):
    """Test that creating a new superuser with no email raises an error"""
    with pytest.raises(ValueError) as err:
